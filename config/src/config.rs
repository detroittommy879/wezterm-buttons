use crate::background::{
    BackgroundLayer, BackgroundSize, BackgroundSource, BlendMode, Gradient,
    GradientOrientation, Interpolation,
};
...
fn w111erd_default_background_layer() -> BackgroundLayer {
    BackgroundLayer {
        source: BackgroundSource::Gradient(Gradient {
            orientation: GradientOrientation::Linear { angle: Some(-35.0) },
            colors: vec![
                "#09131f".to_string(),
                "#12324a".to_string(),
                "#2f647d".to_string(),
        ...
        width: BackgroundSize::Dimension(Dimension::Percent(1.0)),
        height: BackgroundSize::Dimension(Dimension::Percent(1.0)),
    }
}
...
impl Config {
    ...
    pub fn compute_extra_defaults(&self, config_path: Option<&Path>) -> Self {
        ...
        if cfg.background.is_empty() {
            // First try loading gradient from external TOML config
            if let Some(layer) = crate::w111erd_config::get_w111erd_gradient() {
                cfg.background.push(layer);
            } else {
                cfg.background.push(w111erd_default_background_layer());
            }
        }
