...
pub const CUSTOM_TOP_BAR_ENABLED: bool = true;
pub const CUSTOM_LEFT_DOCK_ENABLED: bool = true;
...
impl TermWindow {
    ...
    pub fn top_bar_pixel_height(&self) -> f32 {
        Self::top_bar_pixel_height_impl(&self.render_metrics)
    }
    ...
    pub fn left_dock_pixel_width(&self) -> f32 {
        Self::left_dock_pixel_width_impl(&self.render_metrics)
    }

    pub fn top_tab_bar_pixel_height(&self) -> f32 {
    ...
    pub fn bottom_tab_bar_pixel_height(&self) -> f32 {
        if self.show_tab_bar && self.config.tab_bar_at_bottom {
            self.tab_bar_pixel_height().unwrap_or(0.0)
        } else {
            0.0
    ...
    pub fn top_chrome_pixel_height(&self) -> f32 {
        self.top_tab_bar_pixel_height() + self.top_bar_pixel_height()
    }

    pub fn left_chrome_pixel_width(&self) -> f32 {
        self.left_dock_pixel_width()
    }

    pub fn bottom_chrome_pixel_height(&self) -> f32 {
        self.bottom_tab_bar_pixel_height()
    }

    pub fn top_bar_pixel_y(&self) -> f32 {
        self.get_os_border().top.get() as f32 + self.top_tab_bar_pixel_height()
    }
}
...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        let top_bar_height = Self::top_bar_pixel_height_impl(&render_metrics) as usize;
        let left_dock_width = Self::left_dock_pixel_width_impl(&render_metrics) as usize;
        ...
        let mut dimensions = Dimensions {
            pixel_width: (terminal_size.pixel_width + padding_left + padding_right + left_dock_width)
                as usize,
            ...
            if config.use_resize_increments {
                window.set_resize_increments(
                    ResizeIncrementCalculator {
                        ...
                        border: border,
                        chrome_height: tab_bar_height + top_bar_height,
                    }
                    .into(),
                );
            }
