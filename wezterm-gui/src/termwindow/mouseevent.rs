use crate::tabbar::preset_button_command;
use crate::tabbar::TabBarItem;
use crate::termwindow::{
    GuiWin, LeftDockItem, MouseCapture, PositionedSplit, ScrollHit, TermWindowNotif, TopBarItem,
...
impl super::TermWindow {
    ...
    fn leave_ui_item(&mut self, item: &UIItem) {
        match item.item_type {
            ...
            UIItemType::TopBar(_) | UIItemType::LeftDock(_) => {}
    ...
    fn enter_ui_item(&mut self, item: &UIItem) {
        match item.item_type {
            UIItemType::TabBar(_) | UIItemType::TopBar(_) | UIItemType::LeftDock(_) => {}
            UIItemType::CloseTab(_)
            | UIItemType::AboveScrollThumb
            | UIItemType::BelowScrollThumb
            | UIItemType::ScrollThumb
    ...
    pub fn mouse_event_impl(&mut self, event: MouseEvent, context: &dyn WindowOps) {
        ...
        let first_line_offset = self.top_chrome_pixel_height() as isize + border.top.get() as isize;
        ...
        let x = (event
            .coords
            .x
            .sub((self.left_chrome_pixel_width() + padding_left + border.left.get() as f32) as isize)
            .max(0) as f32)
    ...
    fn drag_scroll_thumb(
        ...
        let border = self.get_os_border();
        let y_offset = self.top_chrome_pixel_height() + border.top.get() as f32;
        ...
        let row = ScrollHit::thumb_top_to_scroll_top(
            ...
            self.dimensions.pixel_height.saturating_sub(
                y_offset as usize
                    + border.bottom.get()
                    + self.bottom_chrome_pixel_height() as usize,
            ),
    ...
    fn mouse_event_ui_item(
        ...
        match item.item_type {
            UIItemType::TopBar(item) => {
                self.mouse_event_top_bar(item, event, context);
            }
            UIItemType::LeftDock(item) => {
                self.mouse_event_left_dock(item, event, context);
            }
    ...
    fn mouse_event_top_bar(
        ...
        if matches!(event.kind, WMEK::Press(MousePress::Left)) {
            ...
            match item {
                ...
                TopBarItem::SplitHorizontal => {
                    self.perform_key_assignment(&pane, &KeyAssignment::SplitHorizontal(SpawnCommand::default()))
                        .ok();
                }
                TopBarItem::SplitVertical => {
                    self.perform_key_assignment(&pane, &KeyAssignment::SplitVertical(SpawnCommand::default()))
                        .ok();
                }
                TopBarItem::SplitAuto => {
                    let dims = pane.get_dimensions();
                    let action = if dims.pixel_width >= dims.pixel_height {
                ...
                TopBarItem::Launcher => {
                    self.perform_key_assignment(&pane, &KeyAssignment::ShowLauncher)
                        .ok();
                }
                TopBarItem::QuickSelect => {
                    self.perform_key_assignment(&pane, &KeyAssignment::QuickSelect)
                        .ok();
                }
            }
        }
        ...
        if matches!(event.kind, WMEK::Press(MousePress::Left)) {
            ...
            match item {
                LeftDockItem::PresetButton(idx) => {
                    if let Some(command) = preset_button_command(idx) {
                        let _ = pane.writer().write_all(command.as_bytes());
                    }
                }
            }
        }
    ...
    pub fn mouse_event_tab_bar(
        ...
        match event.kind {
            WMEK::Press(MousePress::Left) => match item {
                ...
                TabBarItem::PresetButton(idx) => {
                    if let (Some(command), Some(pane)) = (
                        preset_button_command(idx),
