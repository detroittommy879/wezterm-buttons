use crate::termwindow::{PaneInformation, TabInformation, UIItem, UIItemType, CUSTOM_TOP_BAR_ENABLED};
use config::{w111erd_config, ConfigHandle, TabBarColors};
...
lazy_static! {
    static ref PRESET_BUTTONS: Mutex<Vec<(String, String)>> = {
        Mutex::new(w111erd_config::w111erd_preset_buttons()
            .into_iter()
            .map(|b| (b.label, b.command))
            .collect())
    };
}

fn fallback_preset_buttons() -> Vec<(String, String)> {
    vec![
        ("server-amp".to_string(), "ssh -i C:\\Users\\Basic\\.ssh\\oracle-amp.key ubuntu@193.122.157.24\r".to_string()),
        ("server-backwuu".to_string(), "ssh -i C:\\Users\\Basic\\.ssh\\oracle-050525backwuu.key ubuntu@129.213.87.157\r".to_string()),
        ("tail access".to_string(), "tail -f /var/log/apache2/access.log\r".to_string()),
        ("tail buttonscli".to_string(), "tail -f /var/log/apache2/buttonscli_access.log\r".to_string()),
        (" apt update &&".to_string(), " sudo apt update && sudo apt upgrade -y\r".to_string()),
        ("PWD".to_string(), "pwd\r".to_string()),
    ]
}

/// Returns preset button count from TOML config, or 6 if none loaded (for backwards compat)
pub fn preset_button_count() -> usize {
    let loaded = w111erd_config::w111erd_preset_buttons();
    if !loaded.is_empty() {
        return loaded.len();
    }
    6
...
pub fn preset_button_command(idx: usize) -> Option<String> {
    let buttons = PRESET_BUTTONS.lock().ok()?;
    if !buttons.is_empty() {
        return buttons.get(idx).map(|item| item.1.clone());
    }
    drop(buttons);
    fallback_preset_buttons().get(idx).map(|item| item.1.clone())
}

pub fn preset_button_label(idx: usize) -> Option<String> {
    let buttons = PRESET_BUTTONS.lock().ok()?;
    if !buttons.is_empty() {
        return buttons.get(idx).map(|item| item.0.clone());
    }
    drop(buttons);
    fallback_preset_buttons().get(idx).map(|item| item.0.clone())
}
...
impl TabBarState {
    ...
    pub fn new(
        ...
        if !CUSTOM_TOP_BAR_ENABLED {
            let preset_bg = ColorSpec::TrueColor(SrgbaTuple(0.0, 0.0, 0.0, 1.0));
            let preset_fg = ColorSpec::TrueColor(SrgbaTuple(187.0/255.0, 187.0/255.0, 187.0/255.0, 1.0));
            let preset_bg_hover = ColorSpec::TrueColor(SrgbaTuple(51.0/255.0, 51.0/255.0, 51.0/255.0, 1.0));
            let preset_fg_hover = ColorSpec::TrueColor(SrgbaTuple(187.0/255.0, 187.0/255.0, 187.0/255.0, 1.0));
            let mut preset_cell_attrs = CellAttributes::default();
            preset_cell_attrs.set_background(preset_bg);
            preset_cell_attrs.set_foreground(preset_fg);
            let mut preset_cell_hover_attrs = CellAttributes::default();
            preset_cell_hover_attrs.set_background(preset_bg_hover);
            preset_cell_hover_attrs.set_foreground(preset_fg_hover);

            let preset_buttons = PRESET_BUTTONS.lock().unwrap();
            for (idx, preset) in preset_buttons.iter().enumerate() {
                let plain_title = format!(" [{}] ", preset.0);
                let title_template = parse_status_text(&plain_title, preset_cell_attrs.clone());
                let hover = is_tab_hover(mouse_x, x, title_template.len());

                let title = if hover {
                    parse_status_text(&plain_title, preset_cell_hover_attrs.clone())
                } else {
                    title_template
                };
                ...
                let width = title.len();
                items.push(TabEntry {
                    item: TabBarItem::PresetButton(idx),
                    title: title.clone(),
                    x,
                    width,
                });
                line.append_line(title, SEQ_ZERO);
                x += width;
            }
        }
